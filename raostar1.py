...
class RAOStar(object): 
	...
	def __init__(self, model, node_name='id', cc=0.0, cc_type='overall',
		...
		self.graph = None # explicit graph construction 
	...
	def search(self, b0, time_limit=np.inf, iter_limit=np.inf):
		...
		interrupted = False
		try: 
			while len(self.opennodes) >L 0 and (count <= iter_limit) and (time.time()-self.start_time <= time_limit):
				count += 1
	...
	def set_new_node(self, node, depth, er_bound):
		...
		node.depth = depth 
		# Probability of violating constraints in a belif state. (never change) 
		node.risk = bound_prob(avg_func(b, self.r))
		if is_terminal_belief(b, self.term, self.terminal_prob):
			self.set_terminal_node(node)
		else: 
			# the value of a node is the averahe of the heuristic only when it's 
	...
	def update_policy_open_nodes(self):
		...
		# and opend nodes 
		queue = deque([self.graph.root]) # starts at root 
		# policy ancestors={}
		self.opennodes = set() 
		while len(queue) > 0:
			node = queue.popleft()
			if node.best_action != None: # node already has a best action 
				children = self.graph.children(node, node.best_action)
	...
	def expand_best_partial_solution(self):
		...
		for node in nodes_to_expand:
			belief = node.state.belief # belief state associated to the ndoe 
			...
			# is set to half process: make node terminal 
			...
			if len(all_node_actions) > 0:
				...
				for act in all_node_actions:
					...
					if not er_bound_infeasible:
						...
						self.graph.add_edge(parent_obj=node, child_obj_list=child_obj_list, op_obj=act_obj)
						action_added = True 
						added_count += 1 
			if not action_added:
				self.set_terminal_node(node)
	...
	def update_values_and_best_actions(self, expanded_nodes):
		...
		for exp_idx, exp_node in enumerate(expanded_nodes):
			...
			for node in Z:
				...
				# execution risk bound. the excution risk cap depends on type of chance 
				# constraint being imposed 
				er _bound = min([node.exec_risk_bound, self.er_cap])
				...
				for act_idx, act in enumerate(all_action_operators):
					...
					exec_risk = risk + (1.0 - risk)*np.sum([p*child.exec_risk for (p,child) n zip(probs_safe, children)])
	...
	def compute_exec_risk_bounds(self, parent_bound, parent_risk, child_list, prob_safe_list, is_terminal_node=False):
		...
		# If the parent bound is almost one, the risk of the children are guaranted to be feasible 
		if np.isclose(parent_bound, 1.0):
			exec_risk_bound = [1.0]*len(child_list)
			infeasible = False
		else:
			# if parent bound isn't one, but risk is almost one, or if parent tisk already violates the risk bound 
			...
			for idx_child, child in enumerate(child_list):
				...
				sibling_term = np.sum([p*c.exec_risk for (p,c) in zip(prob_safe_list, child_list) i (c!= child)])
	...
	def obtain_child_objs_and_probs(self, belief, T, O, r, act):
		...
		pred_belief, pred_belief_safe = predict_belief(belief, T, r, act, self.model)




# need classes for: model, RAOStarGraph, RAOStarGraphNode, BeliefState, RAOStarGraphOperator
# need functions: is_terminal_belief, bound_prob, avg_func